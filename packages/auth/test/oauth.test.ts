...
import { BetterAuthStrategy } from '../src/strategies/better-auth'

describe('BetterAuthStrategy', () => {
  test('returns user when better-auth session is valid', async () => {
    const strategy = new BetterAuthStrategy({ url: 'http://auth.example' })
    ...
    try {
      ...
      const user = await strategy.authenticate(req, fakeCtx)
      expect(user).toEqual({ id: 'ba-1', email: 'u@example.com' })
    } finally {
      globalThis.fetch = originalFetch
    }
  })
  ...
  test('returns null when better-auth returns non-200', async () => {
    const strategy = new BetterAuthStrategy({ url: 'http://auth.example' })
  ...
  test('custom cookieName is respected', async () => {
    ...
    globalThis.fetch = mock(async (_url: unknown, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify({ user: { id: 'x' }, session: {} }), {
        headers: { 'content-type': 'application/json' },
      })
    }) as typeof fetch
    ...
    try {
      ...
      await strategy.authenticate(req, fakeCtx)
