...
export function HttpCode(statusCode: number) {
  return (_target: unknown, context: ClassMethodDecoratorContext): void => {
    ...
    ;(context.metadata[HTTP_CODE_METADATA] as Record<string, number>)[
...
export function Header(name: string, value: string) {
  return (_target: unknown, context: ClassMethodDecoratorContext): void => {
    ...
    const headers = context.metadata[RESPONSE_HEADERS_METADATA] as Record<
      string,
      [string, string][]
    >
...
export function Redirect(url: string, statusCode = 302) {
  return (_target: unknown, context: ClassMethodDecoratorContext): void => {
    ...
    ;(
      context.metadata[REDIRECT_METADATA] as Record<
        string,
        { url: string; statusCode: number }
      >
    )[context.name as string] = { url, statusCode }
  }
}
