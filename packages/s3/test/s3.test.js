...
describe('S3Module.forRoot', () => {
  test('registers an S3Client as S3_TOKEN', async () => {
    ...
    const client = app.container.resolve(S3_TOKEN)
...
describe('S3Service', () => {
  function makeMockClient() {
    const mockFile = {
      write: mock(async (_data) => 10),
      arrayBuffer: mock(async () => new ArrayBuffer(5)),
      presign: mock(
        (_opts) => `https://s3.example.com/key?exp=${_opts.expiresIn}`,
      ),
      delete: mock(async () => {}),
    }
    const mockClient = {
      file: mock((_key) => mockFile),
    }
  ...
  test('upload writes data and returns bytes written', async () => {
    ...
    const svc = new S3Service(mockClient)
